...
contains
  subroutine sweq(elem,edge1,edge2,edge3,red,par,ithr,nets,nete)
    ...
    use shallow_water_mod, only : tc1_init_state, tc2_init_state, tc5_init_state, tc6_init_state, tc5_invariants, &
         ...
         toy_chemistry_forcing , planar_dv_init_state, planar_dv_invariants
    !-----------------
...
!   call test_subcell_dss_fluxes_again(elem,hybrid,deriv,nets,nete)
    ...
    ! =================================================================
       ...
       if (runtype .eq. 1) then
          ...
          else if (test_case(1:5) == "dbvor") then
             if (hybrid%masterthread) print *,"Restarting dbvor..."
             call planar_dv_init_state(elem,nets,nete,pmean,deriv)
             call planar_dv_invariants(elem,90,tl,pmean,edge2,deriv,hybrid,nets,nete)
          end if
          !============================
          ...
          if (test_case(1:5) == "swtc1") then
             ...
             call sj1_errors(elem,7,tl,pmean,"ref_sj1_imp",simday,hybrid,nets,nete,par)
              ...
              if (hybrid%masterthread) print *,"initializing dbvor..."
              call planar_dv_init_state(elem,nets,nete,pmean,deriv)
              call planar_dv_invariants(elem,90,tl,pmean,edge2,deriv,hybrid,nets,nete)
...
!!! RDL            call tc8_invariants(90, tl, pmean, edge2, deriv, hybrid, nets, nete)
  ...
  subroutine sweq_rk(elem, edge1,edge2,edge3,red,par,ithr,nets,nete)
    ...
    use shallow_water_mod, only : tc1_init_state, tc2_init_state, tc5_init_state, &
         ...
         kmass_swirl, toy_chemistry_forcing, planar_dv_init_state, planar_dv_invariants
    !-----------------
#ifdef PIO_INTERP
    use interp_movie_mod, only : interp_movie_init, interp_movie_output, interp_movie_finish
