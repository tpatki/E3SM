...
! i put subcase 2 center data here cause this is what pl uses for paper
  ...
  ! ============================================
  ! Shallow Water Test Case Parameters
  ! Parameters for test case dbvor,
  ...
  public  :: planar_dv_init_state  ! Initialize planar double vortex test case
  private :: planar_dv_geopotential
  private :: planar_dv_velocity
  public  :: planar_dv_invariants
...
subroutine toy_chemistry_forcing(elem, nets, nete, tl, dt)
  ...
  ! ===========================================
  !
  ! planar_dv_init_state:
  !
  ! Initialize planar double vortex test case
  !
  ! ===========================================

  subroutine planar_dv_init_state(elem,nets,nete,pmean,deriv)
    type(element_t), intent(inout) :: elem(:)
    ...
    do ie=nets,nete
       ...
       do k=1,nlev
          elem(ie)%state%p(:,:,k,n0)=planar_dv_geopotential(elem(ie)%spherep(:,:))
          elem(ie)%state%p(:,:,k,nm1)=elem(ie)%state%p(:,:,k,n0)
          elem(ie)%state%p(:,:,k,np1)=0.0D0

          elem(ie)%state%v(:,:,:,k,n0)=planar_dv_velocity(elem(ie)%spherep(:,:),elem(ie)%Dinv)
          elem(ie)%state%v(:,:,:,k,nm1)=elem(ie)%state%v(:,:,:,k,n0)
          elem(ie)%state%v(:,:,:,k,np1)=0.0D0
       end do
    end do

  end subroutine planar_dv_init_state
  ...
  function planar_dv_velocity(sphere,D) result(v)
  ...
  end function planar_dv_velocity

  ! ===========================================
  !
  ! planar_dv_geopotential:
  !
  ! Set geopotential to initial values
  !
  ! ===========================================

  function planar_dv_geopotential(sphere) result(p)
  ...
  end function planar_dv_geopotential

  ! ======================================================
  ! planar_dv_invariants:
  ! compute invariants on page 221 of Williamson, et. al.
  ! on a non-staggered grid.
  ! ======================================================
  subroutine planar_dv_invariants(elem, iounit,tl,pmean,edge2,deriv,hybrid,nets,nete)
    type(element_t), intent(inout) :: elem(:)
    ...
    if (tl%nstep == 0) then
       if (hybrid%masterthread) then
          open(iounit+0,file=TRIM(output_prefix)//"dbvor.mass",status="unknown",form="formatted")
          open(iounit+1,file=TRIM(output_prefix)//"dbvor.energy",status="unknown",form="formatted")
          open(iounit+2,file=TRIM(output_prefix)//"dbvor.penst",status="unknown",form="formatted")
          open(iounit+3,file=TRIM(output_prefix)//"dbvor.vort",status="unknown",form="formatted")
          open(iounit+4,file=TRIM(output_prefix)//"dbvor.div",status="unknown",form="formatted")
          open(iounit+5,file=TRIM(output_prefix)//"dbvor.rmsdiv",status="unknown",form="formatted")
       endif
    endif
  ...
  end subroutine planar_dv_invariants
