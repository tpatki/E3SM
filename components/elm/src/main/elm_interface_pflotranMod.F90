...
!------------------------------------------------------------------------------------------!
  ...
  !--------------------------------------------------------------------------------------------
    ...
    subroutine clm_pf_run(elm_interface_data, bounds, filters, ifilter)
...
#ifdef CLM_PFLOTRAN
    call clm_pf_BeginCBalance(elm_interface_data, bounds, filters, ifilter)
    call clm_pf_BeginNBalance(elm_interface_data, bounds, filters, ifilter)
    ...
    if (nstep > 1 )then
        call clm_pf_CBalanceCheck(elm_interface_data, bounds, filters, ifilter)
        call clm_pf_NBalanceCheck(elm_interface_data, bounds, filters, ifilter)
    end if
#else
    call pflotran_not_available(subname)
#endif
...
!-------------------------------------------------------------------------------------
  ...
  !-----------------------------------------------------------------------
    ...
    if (isinitpf) then
      call calc_nestep()  ! nestep
       ...
       call get_clm_soil_dimension(elm_interface_data, bounds)

       ! Currently always set soil hydraulic/BGC properties from CLM to PF
       call get_clm_soil_properties(elm_interface_data, bounds, filters)
       ...
       call get_clm_soil_th(elm_interface_data, .not.initth_pf2clm, .not.initth_pf2clm, bounds, filters, ifilter)
    ...
    if (.not. isinitpf .and. (.not.pf_tmode .or. .not.pf_hmode)) then    ! always initialize from CLM to pF, if comment out this 'if'block
       call get_clm_soil_th(elm_interface_data, .TRUE., .TRUE., bounds, filters, ifilter)
    ...
    if (.not.pf_frzmode) then
        call get_clm_iceadj_porosity(elm_interface_data, bounds, filters, ifilter)
    ...
    if (pf_tmode) then
        call get_clm_bceflx(elm_interface_data, bounds, filters, ifilter)
    ...
    if (pf_hmode) then      !if coupled 'H' mode between CLM45 and PFLOTRAN
        call get_clm_bcwflx(elm_interface_data, bounds, filters, ifilter)
    ...
    !      (this will be easier to maintain balance error-free)
        ...
        call get_clm_bgc_conc(elm_interface_data, bounds, filters, ifilter)
    ...
    ! (4b) bgc rate (fluxes) from CLM to PFLOTRAN
        call get_clm_bgc_rate(elm_interface_data, bounds, filters, ifilter)
  ...
  !
  ! !INTERFACE:
  subroutine get_clm_soil_dimension(elm_interface_data, bounds)
    !
    ! !DESCRIPTION:
...
#include "petsc/finclude/petscvec.h90"
  ...
  !
  ! !INTERFACE:
  subroutine get_clm_soil_properties(elm_interface_data, bounds, filters)
    !
    ! !DESCRIPTION:
...
!     call clm_pf_checkerr(ierr, subname, __FILE__, __LINE__)
  ...
  !
  ! !INTERFACE:
  subroutine get_clm_soil_th(elm_interface_data,initpftmode, initpfhmode, bounds, filters, ifilter)
...
#include "petsc/finclude/petscvec.h90"
  ...
  !
  ! !INTERFACE:
  subroutine get_clm_iceadj_porosity(elm_interface_data, bounds, filters, ifilter)
  !
  ! !DESCRIPTION:
...
!-----------------------------------------------------------------------------
  ...
  !
  ! !INTERFACE:
  subroutine get_clm_bcwflx(elm_interface_data, bounds, filters, ifilter)
  !
  ! !DESCRIPTION:
  !
...
!----------------------------------------------------------------------------
  ...
  !
  ! !INTERFACE:
  subroutine get_clm_bceflx(elm_interface_data, bounds, filters, ifilter)
  !
  ! !DESCRIPTION:
  !
...
!----------------------------------------------------------------------------
  ...
  !BOP
  !
  ! !ROUTINE: get_clm_bgc_conc(elm_interface_data, bounds, filters, ifilter)
  !
  ! !INTERFACE:

  subroutine get_clm_bgc_conc(elm_interface_data, bounds, filters, ifilter)
    use ColumnType          , only : col_pp
    use clm_varctl          , only : iulog
...
!   PetscScalar, pointer :: decomp_ppools_vr_clm_loc(:)      ! (gN/m3) vertically-resolved decomposing (litter, cwd, soil) P pools
  ...
  !
  ! !INTERFACE:
  subroutine get_clm_bgc_rate(elm_interface_data,  bounds, filters, ifilter)
...
!--------------------------------------------------------------------------------------
  subroutine clm_pf_BeginCBalance(elm_interface_data, bounds, filters, ifilter)
    !
    ! !DESCRIPTION:
...
!--------------------------------------------------------------------------------------
  ...
  subroutine clm_pf_BeginNBalance(elm_interface_data, bounds, filters, ifilter)
    !
    ! !DESCRIPTION:
    ! On the radiation time step, calculate the beginning carbon balance for mass
    ! conservation checks.
...
!--------------------------------------------------------------------------------------
  ...
  subroutine clm_pf_CBalanceCheck(elm_interface_data,bounds, filters, ifilter)
    !
    ! !DESCRIPTION:
...
!--------------------------------------------------------------------------------------
  ...
  subroutine clm_pf_NBalanceCheck(elm_interface_data,bounds, filters, ifilter)
    !
    ! !DESCRIPTION:
    ! On the radiation time step, perform carbon mass conservation check for column and pft
    !
